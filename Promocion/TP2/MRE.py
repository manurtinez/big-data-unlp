...
class _Context:
    ...
    def finish(this):
        ...
        for t in this.__result:
            f.write(this.__flat(
                t[0]) + OutputKeyValueSeparator + this.__flat(t[1]) + "\n")
        f.close()
    ...
    def write(this, k, v):
        if (this.__stage == 0):
            # map
            this.__interm = this.__addOrUpdateKey(
                this.__interm, k, v, this.__fShuffleCmp)

        elif (this.__stage == 3):
            # reduce
            if (type(k) == ValuesIterator) or (type(v) == ValuesIterator):
                raise MRE_Exception(
                    "No es posible escribir la lista de valores. Recorrala con un for y escriba los elementos por separado; o use el método next() si sabe que la lista de values solo tiene un elemento.")
...
class _MapIterator:
    ...
    def __initInput(this, ci):
        if ci < len(this.__inputs):
            ...
            if(this.__currentFuncMap is None):
                ...
                this.__files = [obj.name for obj in scandir(
                    this.__currentDir) if obj.is_file()]
        else:
            this.__files = []
    ...
    def __initFile(this, cf):
        if cf < len(this.__files):
            f = open(this.__currentDir + "/" +
                     this.__files[cf], "r", encoding='latin-1')
    ...
    def __next__(this):
        if(this.__currentInput >= len(this.__inputs)):
            ...
            this.__offset = this.__offset + \
                len(this.__lines[this.__currentLine])
...
class ValuesIterator:
    ...
    def __iter__(this):
        if (this.__firstTime):
            ...
            raise MRE_Exception(
                "No es posible recorrer la lista de valores más de una vez. Deberá hacer todas las operaciones dentro de un único for.")
...
class Job:
    ...
    def waitForCompletion(this):
        context = _Context(this.__inputs, this.__interDir, this.__output,
                           this.__fComb, this.__params, this.__fShuffleCmp, this.__fSortCmp)
        this.__map(context)
